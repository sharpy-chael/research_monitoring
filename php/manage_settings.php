...
function createItem($con, $type, $data) {
    switch ($type) {
        ...
        case 'academic-year':
            ...
            if (!validateDate($yearStart) || !validateDate($yearEnd)) {
            ...
            // is_active defaults to TRUE if today falls within the date range,
            // otherwise FALSE — admin can still toggle manually later
            $stmt = $con->prepare("
                ...
                VALUES (
                    ...
                    :year_end,
                    :semester,
                    (CURRENT_DATE BETWEEN :year_start2 AND :year_end2)
                )
            ");
            $stmt->execute([
                'year_start'  => $yearStart,
...
function updateItem($con, $type, $data) {
    ...
    switch ($type) {
        ...
        case 'program':
            ...
            $stmt = $con->prepare("
                UPDATE programs SET code = :code, name = :name, description = :description
                WHERE id = :id
            ");
            $stmt->execute(['code' => $code, 'name' => $name, 'description' => $description, 'id' => $itemId]);
        ...
        case 'research-status':
            ...
            $stmt = $con->prepare("
                ...
                SET name = :name, description = :description,
                    color = :color, display_order = :display_order
                WHERE id = :id
            ");
            $stmt->execute([
                'name'          => $name,
...
function toggleStatus($con, $type, $data) {
    ...
    $stmt->execute(['is_active' => $isActive, 'id' => $itemId]);
...
function setActiveAY($con, $data) {
    ...
    if (!$ayId) throw new Exception('Academic year ID is required');
