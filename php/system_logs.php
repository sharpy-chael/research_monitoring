...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'])) {
    try {
        if ($_GET['action'] === 'list') {
            $stmt = $con->query("
                SELECT id, user_id, user_type, action_type, description, ip_address, created_at
                FROM system_logs
            ...
            $stmt = $con->query("
                ...
                LIMIT 100
            ");
            echo json_encode(['success' => true, 'errors' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                fputcsv($output, [$row['id'], $row['user_type'], $row['action_type'], $row['description'], $row['ip_address'], $row['created_at']]);
            }
            fclose($output);
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    try {
        if (isset($_GET['action']) && $_GET['action'] === 'clear_errors') {
            ...
            ")->execute(['user_id' => $user_id, 'user_type' => $_SESSION['role'], 'ip_address' => $_SERVER['REMOTE_ADDR']]);
