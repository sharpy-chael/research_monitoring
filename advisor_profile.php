...
$facultyStmt = $con->prepare("
    SELECT f.id, f.name, f.department, f.email, f.gender, f.images, f.address
    FROM faculties f
    WHERE f.user_id = :user_id
");
$facultyStmt->execute(['user_id' => $advisorUserId]);
$row = $facultyStmt->fetch(PDO::FETCH_ASSOC);
...
<head>
  ...
  <style>
    .edit-modal, .change-password {
      display: none; position: fixed; inset: 0;
      background: rgba(0,0,0,0.45); z-index: 1000;
      align-items: center; justify-content: center;
    }
    .edit-modal.active, .change-password.active { display: flex; }
    .edit-modal-content, .change-modal-content {
    ...
    .edit-modal-content h2, .change-modal-content h2 {
      font-size: 1.25rem; font-weight: 700; color: #8b0000;
      margin-bottom: 22px; text-align: center;
    }
    .edit-modal-content label, .change-modal-content label {
    ...
    .change-modal-content input[type="password"] {
      ...
      border-radius: 7px; font-size: 0.9rem; background: #f9f9f9;
      box-sizing: border-box; transition: border-color 0.2s;
    }
    .edit-modal-content input:focus, .change-modal-content input:focus {
      outline: none; border-color: #8b0000; background: #fff;
    }
    .edit-modal-content input[readonly] {
      background: #efefef; color: #888; cursor: not-allowed;
    }
    .edit-modal-content .file-label {
      display: flex; align-items: center; gap: 10px;
      border: 1.5px dashed #ccc; border-radius: 7px;
      padding: 10px 12px; background: #fafafa;
      cursor: pointer; font-size: 0.85rem; color: #777; margin-top: 4px;
    }
    .edit-modal-content input[type="file"] { display: none; }
    .button-row {
      display: flex; gap: 12px; margin-top: 24px; justify-content: flex-end;
    }
    .btn-outline {
      padding: 9px 22px; border: 2px solid #8b0000; background: transparent;
      color: #8b0000; border-radius: 7px; font-weight: 600; cursor: pointer;
      font-size: 0.9rem; transition: background 0.2s, color 0.2s;
    }
    .btn-outline:hover { background: #8b0000; color: #fff; }
    .btn-solid {
      padding: 9px 22px; border: none; background: #8b0000; color: #fff;
      border-radius: 7px; font-weight: 600; cursor: pointer;
      font-size: 0.9rem; transition: background 0.2s;
    }
    .btn-solid:hover { background: #6a0000; }
    .close-btn {
      position: absolute; top: 14px; right: 18px; font-size: 1.4rem;
      cursor: pointer; color: #aaa; line-height: 1; transition: color 0.2s;
    }
...
<main class="profile-wrapper">
  ...
  <section class="profile-details">
    ...
    <div class="gender-row">
      <label><input type="radio" name="gender" disabled value="Male" <?php echo ($row['gender'] ?? '') === 'Male' ? 'checked' : ''; ?>> Male</label>
      <label><input type="radio" name="gender" disabled value="Female" <?php echo ($row['gender'] ?? '') === 'Female' ? 'checked' : ''; ?>> Female</label>
    </div>
    ...
    <div class="form-group">
      <label>Email</label>
      <input type="text" readonly value="<?php echo htmlspecialchars($row['email'] ?? ''); ?>">
    </div>
    <div class="form-group">
      <label>Address</label>
      <input type="text" readonly value="<?php echo htmlspecialchars($row['address'] ?? ''); ?>">
    </div>
    <div class="form-row">
      <div class="form-group">
        <label>Department</label>
        <input type="text" readonly value="<?php echo htmlspecialchars($row['department'] ?? ''); ?>">
      </div>
    </div>
  </section>
</main>
...
<div id="editModal" class="edit-modal">
  <div class="edit-modal-content">
    ...
    <form id="editForm" action="php/advisor_update.php" method="POST" enctype="multipart/form-data">
      ...
      <label>Department</label>
      <input type="text" name="department" value="<?php echo htmlspecialchars($row['department'] ?? ''); ?>" placeholder="Enter the Department" required>

      <label>Email</label>
      <input type="email" name="email" value="<?php echo htmlspecialchars($row['email'] ?? ''); ?>" placeholder="Enter your email" required>

      <label>Address</label>
      <input type="text" name="address" value="<?php echo htmlspecialchars($row['address'] ?? ''); ?>" placeholder="Enter your Address" required>
